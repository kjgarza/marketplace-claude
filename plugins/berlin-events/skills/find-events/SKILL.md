...
Iterate the typed source registry in `scripts/sources.ts` and dispatch each source through the correct strategy (Readability, generic Playwright, or per-source extractor). Run in parallel where possible.
...
The dispatcher prints plain text for Readability/Playwright sources and JSON `Event[]` for source-extractor sources (tip-berlin, gropius-bau, mitvergnuegen); qurl accepts either.
