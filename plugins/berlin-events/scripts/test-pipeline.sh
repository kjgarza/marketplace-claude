...
# 3. Pipes the output into qurl under source=berlin-events
...
ingest_source() {
  ...
  content=$("$BUN" run "$SCRIPT_DIR/extract-events.ts" "$slug" 2>/dev/null) || {
  ...
  if echo "$content" | qurl add "$url" --source berlin-events --tags "$tag" 2>/dev/null; then
    echo "OK [$tag]"
