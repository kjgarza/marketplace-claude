...
INPUT="$(cat)"
SESSION_ID="$(printf '%s' "$INPUT" | /usr/bin/python3 -c 'import sys,json;print((json.load(sys.stdin) or {}).get("session_id",""))' 2>/dev/null)"
...
/usr/bin/python3 "$(dirname "$0")/tw-stop-decision.py" "$ACTIVE_COUNT" "$ACTIVE_LIST"
exit 0
